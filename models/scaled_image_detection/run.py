...
result_base_path = home_directory + "edge-computing/models/scaled_image_detection/result"
total_experiment = 1
...
def load_labeled_data():
    labels_path = labels_base_path + "/" + "labels.json"
...
def run_experiment(labeled_data, exp_id):
    ...
    for image_id in labeled_data:
        ...
        metric = {}
        labeled_objects = get_labeled_objects(labeled_data[image_id])
        
        original_image_path = images_base_path + "/" + image_id + ".jpg"
        oi_inferred_objects = get_inferred_objects(original_image_path)
        metric["original"] = get_inferred_image_metrics(labeled_objects, oi_inferred_objects)
        ...
        for scaling_factor in scaling_factors:
            ...
            for ip_method in interpolation_methods:
                ...
                pi_inferred_objects = get_inferred_objects(processed_image_path)
                os.remove(processed_image_path)
                interpolation_method_metric[ip_method] = get_inferred_image_metrics(labeled_objects, pi_inferred_objects)
