...
resolutions = ['160x90', '160x100', '160x120', '240x120', '320x180', '320x200', '320x240', '480x270', '480x300', '480x360', '640x360', '640x400', '640x480', '800x450', '800x500', '800x600', '1024x576', '1024x640', '1024x768', '1280x720', '1280x800', '1280x960', '1440x900', '1440x1080', '1920x1080']
...
def transfer_original_image(exp_id):
    ...
    for resolution in resolutions:
        for frame in frames:
            ...
            completed_data += len(image_files)
            logger.info(str(exp_id)  + "-> DONE :::: " + str((completed_data / total_data) * 100) + "%")
    return results



for i in range(1, total_experiment): 
